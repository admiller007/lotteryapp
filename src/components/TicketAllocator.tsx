...
import { allocateTickets } from '@/lib/firebaseService';
...
export default function TicketAllocator({ prizeId }: TicketAllocatorProps) {
  ...
  const handleAllocate = async () => {
    ...
    try {
      console.log('Allocating tickets:', { prizeId, userId: currentUser.id, userName: currentUser.name, tickets: numTickets });
      
      // Save to Firebase first
      await allocateTickets({
        lotteryId: 'default', // Using default lottery for now
        prizeId,
      ...
      console.log('Successfully saved to Firebase, updating local state');
      ...
      toast({ 
        ...
        description: "Failed to save allocation to Firebase. Please try again.", 
        variant: "destructive" 
      });
    }
  };
